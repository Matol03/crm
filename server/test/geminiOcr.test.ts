...
import type { GeminiVisionTransport } from '../src/ocr/gemini.js';

describe('GeminiOcrClient', () => {
  it('short-circuits on pre-supplied fixture text (no vision call)', async () => {
    let called = false;
    const transport: GeminiVisionTransport = async () => {
      called = true;
  ...
  it('calls the vision transport with image bytes and returns trimmed text', async () => {
    const transport: GeminiVisionTransport = async (bytes, mime) => {
